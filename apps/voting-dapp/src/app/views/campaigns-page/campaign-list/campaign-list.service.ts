...
import { CampaignModel } from '../campaign-preview/campaign-preview.service';
...
export class CampaignListService extends BaseContractService {
  constructor(
    private readonly web3AccountService: Web3AccountService,
  ...
  getCampaignsList$(): Observable<CampaignListItem[]> {
    return this.__getData$<GetCampaignsListDto>('getCampaigns()').pipe(
      map((result: GetCampaignsListDto) => {
        return result._addressesArray.map(
          (address, index) =>
            ({
              address,
  ...
  getAllCampaigns$() {
    return this.__getData$<CampaignModel[]>(
      'getAllCampaigns(uint256,uint256)',
      0,
      1
    );
  }
