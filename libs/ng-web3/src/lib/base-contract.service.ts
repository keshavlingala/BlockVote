import { Contract, EventData, SendOptions } from 'web3-eth-contract';
...
export class BaseContractService {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected __getData$<T>(
    methodName: string,
    ...params: unknown[]
  ): Observable<T> {
    this._checkContractMethod(methodName);
    return new Observable((observer) => {
      this._contract?.methods[methodName](...params)
        .call()
        .then((data: T) => {
          console.log(`${methodName}, Params: ${params}`, data);
          observer.next(data);
          observer.complete();
        })
        .catch((err: unknown) => {
  ...
  protected __sendData$(
    ...
    return new Observable((observer) => {
      this._contract?.methods[methodName](...params)
        .send(sendOptions)
        .then(() => {
          console.log(`${methodName}, ${params}`, 'Transaction sent');
          observer.next();
          observer.complete();
        })
        .catch((err: unknown) => {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected __getBaseEvents$<T>(
    eventName: string
  ): Observable<{ [p: string]: unknown }> {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  protected __getEvents$<T>(
    eventName: string,
    mapFunc: (eventValues: { [p: string]: unknown }) => T
  ): Observable<T> {
  ...
  private _checkContractMethod = (methodName: string) =>
    this._checkContract('method', methodName);
  private _checkContractEvent = (eventName: string) =>
    this._checkContract('event', eventName);

  private _checkContract(
    checkType: 'event' | 'method',
    methodOrEventName: string
  ): true {
    if (!this._contract) {
      throw new Error(
        'Contract not initialized. Please call __initialize method first.'
      );
    }
    ...
    if (
      ...
      throw new Error(
        `There is no ${checkType} with given name at given ABI file`
      );
    }
